...
function repairPlan(current) {
  ...
  const block = managedBlock(newline);
  return {
    configContent: `${current.config.text.slice(0, current.block.start)}${block}${current.config.text.slice(current.block.end)}`,
    stateContent: `${JSON.stringify({
      ...current.state.state,
...
async function enable(location, options) {
  ...
  const repairing = preview.status === "user-modified";
  ...
  try {
    ...
    const expectedStatus = repairing ? "user-modified" : "disabled";
