...
async function planLegacySkillMigration(plan, state) {
  const { location } = plan;
  const migrated = new Set();
  for (const name of ["skill", "planning"]) {
    if (!state.files[name]) continue;
    const legacyPath = location[name === "skill" ? "legacySkill" : "legacyPlanning"];
    await validateOperationPath(location, legacyPath);
    const [legacy, current] = await Promise.all([
      readText(legacyPath),
      readText(location[name])
    ]);
    if (!legacy.exists) continue;
    if (current.exists && current.text !== legacy.text) {
      failPlan(plan, `${location[name]} already exists with different content; legacy skill was preserved`);
      return migrated;
    }
    if (!current.exists) addWrite(plan, location[name], legacy.text, "create", `${name} migration`);
    addDelete(plan, legacyPath, legacyPath);
    migrated.add(name);
  }

  const legacyCreatedDirs = state.createdDirs
    .filter((directory) => isWithin(directory, location.legacySkillsRoot));
  state.createdDirs = state.createdDirs
    .filter((directory) => !isWithin(directory, location.legacySkillsRoot));
  for (const directory of legacyCreatedDirs.sort((left, right) => right.length - left.length)) {
    addRemoveDirectory(plan, directory);
  }
  return migrated;
}

function installedManagedPath(location, name, legacySkills) {
  if (!legacySkills || !["skill", "planning"].includes(name)) return location[name];
  return location[name === "skill" ? "legacySkill" : "legacyPlanning"];
}
...
async function planInstall(scope, flags) {
  ...
  const migratedSkills = loaded.legacySkills
    ? await planLegacySkillMigration(plan, state)
    : new Set();
  ...
  for (const name of MANAGED_FILE_NAMES) {
    if (migratedSkills.has(name)) continue;
...
async function planUninstall(scope) {
  ...
  for (const name of MANAGED_FILE_NAMES) {
    const tracked = state.files[name];
    if (!tracked) continue;
    const currentPath = installedManagedPath(location, name, loaded.legacySkills);
    const label = loaded.legacySkills && ["skill", "planning"].includes(name) ? currentPath : name;
    if (loaded.legacySkills && ["skill", "planning"].includes(name)) {
      await validateOperationPath(location, currentPath);
    }
    const current = await readText(currentPath);
    if (!current.exists) addMessage(plan, "skip", label, "already missing");
    else if (digest(Buffer.from(current.text, "utf8")) === tracked.hash) addDelete(plan, currentPath, label);
    else addMessage(plan, "preserve", label, "user-modified");
...
async function validateJournal(raw, location) {
  ...
  for (let index = 0; index < raw.operations.length; index += 1) {
    ...
    if (!isWithin(operation.path, location.codexHome) && !isWithin(operation.path, location.legacySkillsRoot)) {
      throw new Error(`unsafe transaction path: ${operation.path}`);
    }
    await validateOperationPath(location, operation.path);
...
async function prepareJournal(plan) {
  ...
  for (let index = 0; index < transactionalOperations.length; index += 1) {
    ...
    await validateOperationPath(plan.location, operation.path);
    let before;
    let snapshot = null;
  ...
  try {
    ...
    if (transactionalOperations.length) {
      ...
      for (let index = 0; index < transactionalOperations.length; index += 1) {
        ...
        await validateOperationPath(plan.location, transactionalOperations[index].path);
...
function getManagedFast(content) {
  const matches = [...content.matchAll(/^# codex-model-router-fast = (true|false)\s*$/gm)];
...
async function doctor(scope) {
  ...
  for (const name of AGENT_NAMES) {
    ...
    try {
      ...
      const expectation = AGENT_EXPECTATIONS[name];
      const invalid = Object.entries(expectation)
        .find(([key, value]) => key !== "fast" && getString(values, key) !== value)
        ?? (typeof expectation.fast === "boolean" && getManagedFast(current.text) !== expectation.fast
          ? ["fast", expectation.fast]
          : null);
  ...
  for (const [name, expectation] of Object.entries(SKILL_EXPECTATIONS)) {
    const currentPath = installedManagedPath(location, name, loaded.legacySkills);
    if (loaded.legacySkills) await validateOperationPath(location, currentPath);
    const current = await readText(currentPath);
    if (!current.exists) {
      healthy = doctorStatus(messages, "missing", name) && healthy;
      continue;
