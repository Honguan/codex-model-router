...
function managedBlock(newline) {
  const settings = Object.entries(EXPERIMENTAL_V2_SETTINGS).map(([key, value]) => {
    const tomlValue = typeof value === "string" ? JSON.stringify(value) : String(value);
...
function legacyManagedBlock(newline) {
  return [
    START_MARKER,
    "[features.multi_agent_v2]",
    "hide_spawn_agent_metadata = false",
...
async function inspect(location) {
  ...
  const newline = newlineFor(config.text);
  if (block.content === legacyManagedBlock(newline)) {
    return { status: "managed", legacyManaged: true, state, config, block };
  }
  if (block.content !== managedBlock(newline)) {
    return { status: "user-modified", detail: "managed V2 block changed", state, config, block };
  }
...
async function enable(location, options) {
  ...
  if (preview.status === "managed" && !preview.legacyManaged) {
  ...
  const repairing = preview.legacyManaged || ["user-modified", "missing"].includes(preview.status);
...
async function status(location, options) {
  ...
  const [label, detail] = current.legacyManaged
    ? ["user-modified", "managed V2 block changed"]
    : mapping[current.status] || ["invalid", current.status];
  if (!(options.quietIfDisabled && current.status === "disabled")) print(options.output, label, detail);
  return current.legacyManaged
    ? 1
    : ["managed", "disabled", "unmanaged"].includes(current.status) ? 0 : 1;
}
