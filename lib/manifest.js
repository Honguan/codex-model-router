...
Role action gate: the primary supplies the current stage, Luna mode, workspace, and applicable canonical action IDs. Obey only that snapshot. The full recovery, escalation, PLAN, and Luna interaction contracts are owned by the model-router skill; do not duplicate them in role output. No role may self-approve, make architecture or requirement decisions outside its role, expose secrets, or broaden a delegated action.
`;
