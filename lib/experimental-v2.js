...
async function locationFor(options, global) {
  ...
  const rawCodexHome = global ? resolve(env.CODEX_HOME || join(home, ".codex")) : join(cwd, ".codex");
  const codexHome = await canonicalize(rawCodexHome);
...
function appendBlock(original) {
  ...
  const body = original.slice(original.charCodeAt(0) === 0xfeff ? 1 : 0);
  let separator = "";
  if (body.length > 0) {
