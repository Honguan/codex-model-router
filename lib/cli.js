...
const FAST_OPTION_TO_AGENT = Object.freeze({
  "--terra-fast": "terra",
  "--luna-fast": "luna",
  "--sol-fast": "sol",
  "--no-terra-fast": "terra",
...
Fast: role-specific metadata only; configured true is currently reported as not-supported and never enables global fast_mode.`;
}
...
function parseArgs(argv) {
  ...
  for (let index = 1; index < argv.length;) {
    ...
    const fastAgent = FAST_OPTION_TO_AGENT[option];
    ...
    if (option === "--agent-fast" || option === "--no-agent-fast" || fastAgent) {
      if (token !== option) return { error: `${option} does not accept a value` };
      if (command !== "install") return { error: `${option} is supported only for install` };
      const fast = !option.startsWith("--no-");
      if (fastAgent) {
        if (Object.hasOwn(fastOverrides, fastAgent) && fastOverrides[fastAgent] !== fast) {
          return { error: `conflicting Fast settings for ${fastAgent}` };
        }
        fastOverrides[fastAgent] = fast;
      } else if (allFast !== undefined && allFast !== fast) {
        return { error: "conflicting Fast settings for all agents" };
      } else {
        allFast = fast;
      }
      index += 1;
  ...
  return { command, flags, overrides, allReasoning, fastOverrides, allFast };
}
...
async function installedProfile(scope) {
  ...
  if (!stateText) return { reasoning: {}, fast: {}, content: {}, packageVersion: null, states: {} };

  let state;
  try {
    state = JSON.parse(stateText);
  } catch {
    return { reasoning: {}, fast: {}, content: {}, packageVersion: null, states: {} };
  }
  ...
  try {
    ...
    return { reasoning: {}, fast: {}, content: {}, packageVersion: state.packageVersion ?? null, states: {} };
  }
  ...
  for (const name of AGENT_NAMES) {
    ...
    if (resolve(tracked.path) !== resolve(path) || digest(current) !== tracked.hash) {
      states[name] = "user-modified";
      continue;
    }
    const effort = activeReasoning(current);
    const configuredFast = activeFast(current);
    states[name] = configuredFast === null ? "legacy-managed" : "managed";
    if (effort) reasoning[name] = effort;
    if (configuredFast !== null) fast[name] = configuredFast;
    content[name] = current;
  }
  return { reasoning, fast, content, packageVersion: state.packageVersion ?? null, states };
}

function fastStatus(name, profile) {
  const configured = profile.fast[name] ?? false;
  const effective = configured
    ? WORKFLOW_AGENT_FAST_CONTRACT.configuredTrueEffective
    : WORKFLOW_AGENT_FAST_CONTRACT.configuredFalseEffective;
  return `${name[0].toUpperCase()}${name.slice(1)} fast: configured=${configured}, effective=${effective}, state=${profile.states[name] ?? "uninstalled"}`;
}
...
export async function runCli(argv, options = {}) {
  ...
  try {
    ...
    const installed = await installedProfile(scope);
    if (parsed.command === "status") {
      for (const name of AGENT_NAMES) output(fastStatus(name, installed));
      return 0;
    }
    const selected = { ...DEFAULT_AGENT_REASONING };
    const selectedFast = { ...DEFAULT_AGENT_FAST };
    if (versionAtLeast(installed.packageVersion, "2.1.0")) Object.assign(selected, installed.reasoning);
    Object.assign(selectedFast, installed.fast);
    ...
    if (parsed.allFast !== undefined) {
      for (const name of AGENT_NAMES) selectedFast[name] = parsed.allFast;
    }
    Object.assign(selectedFast, parsed.fastOverrides);
    configureAgentReasoning(selected, installed.content, selectedFast);
