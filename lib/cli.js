...
import { migrateLegacyHomeProjectState } from "./state-scope-migration.js";
...
export async function runCli(argv, options = {}) {
  ...
  const migration = await migrateLegacyHomeProjectState(argv, scoped);
  if (migration.status === "blocked") {
    output(`fail: state scope migration (${migration.detail})`);
    return 1;
  }
  if (migration.status === "needed") {
    if (json) {
      output(JSON.stringify({
        ...
        from: "project",
        to: "global",
        dry_run: Boolean(migration.dryRun)
      }, null, 2));
    } else if (migration.dryRun) {
      output("would-migrate: state scope (project -> global)");
      output("note: rerun without --dry-run to migrate before showing the remaining plan");
    } else {
      output("migration-required: state scope (project -> global); run enable --global or repair --global");
    }
    return migration.dryRun ? 0 : 1;
  }
  if (migration.status === "migrated") output("migrate: state scope (project -> global)");
