...
- Starts only after `prepare-tag` succeeds.
...
8. Confirm `prepare-tag` creates the remote tag before the `release` job starts.
...
A failed `prepare-tag` job creates no tag and performs no npm publication. Fix the reported validation or test failure, then run `prepare-and-release` again with the same requested new tag.
